...
Here are some email addresses to find: [john.doe@example.com](mailto:john.doe@example.com), [jane_doe123@domain.org](
mailto:jane_doe123@domain.org), [support@service.net](mailto:support@service.net), [info@company.co.uk](
mailto:info@company.co.uk), and [contact.us@my-website.com](mailto:contact.us@my-website.com). You might also find 
these tricky: [weird.address+spam@gmail.com](mailto:weird.address+spam@gmail.com), "quotes.included@funny.domain", 
and [this.one.with.periods@weird.co.in](mailto:this.one.with.periods@weird.co.in).
...
def parse():
    pattern = r'(\d{1,2}\D\d{1,2}\D(1|2)(1|2|0)\d{2})'
...
def parse_2():
    ...
    transparent = re.search(pattern, text).group(2)
    print('transparent is:', transparent)


xml = """<?xml version="1.0"?>
<data>
    <info number="1">
        <fact id="fact">When a cat drinks, its tongue - which has tiny barbs on it - scoops the liquid up backwards.</fact>
        <length>92</length>
    </info>
    <info number="2">
...
def parse_xpath_xml():
    ...
    facts = root.findall('.//info[@number="1"]/fact')
    facts = [fact.text for fact in facts]
    # print(facts)

    lengts = root.findall('.//info[2]/length')
    lengts = [length.text for length in lengts]
    # print(lengts)

    lengts = root.findall('.//fact/../length')
    lengts = [length.text for length in lengts]
    print(lengts)
